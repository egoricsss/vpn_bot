...
class CacheHelper:
    ...
    async def connect(cls, url: str, max_connections: int = 10) -> None:
        ...
        try:
            cls._pool = Redis.from_url(url, max_connections=max_connections).connection_pool
    ...
    @classmethod
    def cache(cls, ttl: int = 3600, prefix: str = "cache") -> Callable[[Callable[P, T]], Callable[P, Awaitable[T]]]:
                ...
                try:
                    ...
                    if result is not None and cls._initialized and cls._client:
                        try:
                            ...
                            logger.error(f"Unexpected error saving to Redis: {e}", exc_info=True)
